...
function Home() {
  ...
  async function login() { //an asynchronous function to handle login
    try {
      ...
      if (result === "logged in") {  //if response is 'logged in' (for successful login)
  ...
  async function register() { //asynchronous function to handle registration
    try {
  ...
  return (
    <div>
      <div className="authcontainer">
        ...
        <button className="btn" id="register" onClick={() => register()}>
          register
        </button>
      </div>
    </div>
